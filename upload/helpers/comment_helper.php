...
function commenttable($res, $type = null)
{
    ...
    while ($row = $res->fetch(PDO::FETCH_LAZY)) {
        ...
        if ($_SESSION['edit_users'] == 'no' && $privacylevel == 'strong') {
            print('<td align="left" width="150"><center><a href="'.$config['SITEURL'].'/users/profile?id='.$row['id'].'"><b>' . $postername . '</b></a><br /><i>' . $title . '</i><br /><img width="80" height="80" src="' . $avatar . '" alt="" /><br /><br />Uploaded: ---<br />Downloaded: ---<br />Ratio: ---<br /><br /><a href="$config[SITEURL]/users/profile?id=' . $row["user"] . '"><img src="themes/' . ($_SESSION['stylesheet'] ?: 'dafault') . '/forums/icon_profile.png" border="" alt="" /></a> <a href="$config[SITEURL]/messages/create?id=' . $row["user"] . '"><img src="themes/' . ($_SESSION['stylesheet'] ?: 'dafault') . '/forums/icon_pm.png" border="0" alt="" /></a></center></td>');
        } else {
            print('<td align="left" width="150"><center><a href="'.$config['SITEURL'].'/users/profile?id='.$row['id'].'"><b>' . $postername . '</b></a><br /><i>' . $title . '</i><br /><img width="80" height="80" src="' . $avatar . '" alt="" /><br /><br />Uploaded: ' . $useruploaded . '<br />Downloaded: ' . $userdownloaded . '<br />Ratio: ' . $userratio . '<br /><br /><a href="$config[SITEURL]/users/profile?id=' . $row["user"] . '"><img src="themes/' . ($_SESSION['stylesheet'] ?: 'dafault') . '/forums/icon_profile.png" border="0" alt="" /></a> <a href="/messages/create?id=' . $row["user"] . '"><img src="themes/' . ($_SESSION['stylesheet'] ?: 'dafault') . '/forums/icon_pm.png" border="0" alt="" /></a></center></td>');
        }
