...
if (!$config["MEMBERSONLY"] || $_SESSION['loggedin'] == true) {
    ...
    if ($latestrequestsrecords) {
        foreach ($latestrequestsrecords as $row) {
            ...
            echo "<table cellspacing='0' cellpadding='3' width='100%' border='0'><tr><td width='55%'><small><a style='text-decoration: none;' title='" . $row["parent_cat"] . " : " . $row["cat"] . "'>" . $row["parent_cat"] . " : $smallnamereq </a></small></td><td width='45%'><a style='text-decoration: none;' title='" . $row["request"] . "' href='$config[SITEURL]/request/index?Section=Request_Details&id=$row[id]'>$smallname</a></td></tr></table> \n";
        }
    } else {
