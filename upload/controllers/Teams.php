<?php
  class Teamsview extends Controller {
...
global $site_config, $CURUSER;
 ...
 if ($res->rowCount() == 0)
     show_error_msg(T_("ERROR"), "No teams available, to create a group please contact <a href='/staff'>staff</a>.", 1);
     
 stdhead("Teams View");
 begin_frame("Teams View");
 
 echo '<center>Please <a href="/staff">contact</a> a member of staff if you would like a new team creating</center><br />';
...
<thead><tr>
     <th></th>
     <th>Owner: <?php echo ( $row["username"] ) ? '<a href="/accountdetails?id='.$row["owner"].'">' . class_user($row["username"]) . '</a>' : "Unknown User"; ?> - Added: <?php echo utc_to_tz($row["added"]); ?></th>
 </tr></thead>
 <tbody><tr>
 ...
 <tr>
    ...
    <a href="/accountdetails?id=<?php echo $member[0]; ?>"><?php echo htmlspecialchars($member[1]); ?></a>,
    <?php endforeach; ?>
    </td>
 </tr><tbody>
 </table></div>
