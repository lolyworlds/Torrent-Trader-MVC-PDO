...
class Peers extends Controller
{
    ...
    public function dead()
    {
        ...
        if ($count < 1) {
            show_error_msg("" . T_("NOTHING_FOUND") . "", "<div style='margin-top:10px; margin-bottom:10px' align='center'><font size='2'><i>...No Dead Torrents !</i></font></div>", 1);
        }
        ...
        ?>
            ...
            if ($row["username"]) {
                $owner = "<a href='$config[SITEURL]/users/profile?id=" . $row["owner"] . "'><b>" . $row["username"] . "</b></a>";
            } else {
