...
class Group extends Controller
{
    ...
    public function staff()
    {
        ...
        <?php while ($row = $res->fetch(PDO::FETCH_ASSOC)): if (!isset($table[$row["group_id"]])) {
            continue;
        }?><tr>
	    <td colspan="14" class="newtd"><center><b><?php echo T_($row["level"]); ?></b> <?php if ($row["staff_public"] == "no") {
            echo ("<font color='#ff0000'>[" . T_("HIDDEN FROM PUBLIC") . "]</font>");
        }
        ?><center></td>
	    </tr>
	    <tr>
