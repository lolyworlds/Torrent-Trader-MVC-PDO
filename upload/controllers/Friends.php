...
loggedinonly();
	...
	$res = DB::run("SELECT f.friendid as id, u.username AS name, u.class, u.avatar, u.title, u.enabled, u.last_access FROM friends AS f LEFT JOIN users as u ON f.friendid = u.id WHERE userid=$userid ORDER BY name");
	if ($res->rowCount() == 0) {
		...
		while ($friend = $res->fetch(PDO::FETCH_ASSOC)) {
			...
			<a href=friends?id=$userid&action=delete&type=friend&targetid=".$friend['id']."><img src=$site_config[SITEURL]/images/delete.png title=Remove border=0></a>
	...
	$res = DB::run("SELECT b.blockid as id, u.username AS name, u.enabled, u.last_access FROM blocked AS b LEFT JOIN users as u ON b.blockid = u.id WHERE userid=$userid ORDER BY name");
	if ($res->rowCount() == 0) {
		...
		while ($block = $res->fetch(PDO::FETCH_ASSOC)) {
			if ($i % 6 == 0)
				$blocked .= "<tr>";
				$blocked .= "<td style='border:none; padding:4px; spacing:0px'><a href=$site_config[SITEURL]/users/profile?id=".$block['id']."><b>".class_user_colour($block['name'])."</b></a> <a href=friends?id=$userid&action=delete&type=block&targetid=".$block['id']."><img src=images/delete.png title=Remove border=0></a></td>";
			if ($i % 6 == 5)
				$blocked .= "</tr>";
			$i++;
		}
...
public function add(){
	...
	loggedinonly();
			...
			$r = DB::run("SELECT id FROM $table_is WHERE userid=$CURUSER[id] AND $field_is=$targetid");
			if ($r->rowCount() == 1)
				show_error_msg("Error", "User ID $targetid is already in your $table_is list.", 1);
			
			$test = DB::run("INSERT INTO $table_is VALUES (0,$CURUSER[id], $targetid)");
			var_dump($test);
	...
	public function delete(){
		...
		if ($type == 'friend') {
			$stmt = DB::run("DELETE FROM friends WHERE userid=$CURUSER[id] AND friendid=$targetid");
			if ($stmt->rowCount() == 0)
				show_error_msg("Error", "No friend found with ID $targetid", 1);
			$frag = "friends";
		} elseif ($type == 'block') {
			$stmt = DB::run("DELETE FROM blocked WHERE userid=$CURUSER[id] AND blockid=$targetid");
			if ($stmt->rowCount() == 0)
