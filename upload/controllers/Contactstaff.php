...
<?php
    ...
    if ($error_msg != "") {
        ...
        echo $error_msg . "</h3></br></br><a href=\"$config[SITEURL]/contactstaff/contactstaff\">Back</a></center>";
    } else {
        $added = get_date_time();
        $userid = $_SESSION['id'];
        $REQ = DB::run("INSERT INTO staffmessages (sender, added, msg, subject) VALUES(?,?,?,?)", [$userid, $added, $msg, $sub]);
        if ($REQ) {
            echo "<center><h3 style=\"color:green;\">Your message has been sent. We will reply as soon as possible.</h3></br></br><a href=\"$config[SITEURL]/index\">Back</a></center>";
        } else {
            echo "<center><h3 style=\"color:red;\">You send the wrong message. try again later</h3></br></br><a href=\"$config[SITEURL]/index\">Back</a></center>";
        }
    }
}
end_frame();
...
");
        ...
        while ($arr = $res->fetch(PDO::FETCH_ASSOC)) {
            ...
            $pmid = $arr["id"];
            print("<tr>
<td><a href=" . $config["SITEURL"] . "/contactstaff/staffbox?action=viewpm&pmid=$pmid><b>$arr[subject]</b></td>
<td><a href=" . $config["SITEURL"] . "/users/read?id=$arr[sender]><b>".class_user_colour($arr['username'])."</b></a></td>
<td>$arr[added]</td><td align=left>$answered</td>");

 print("<td><input type=\"checkbox\" name=\"setanswered[]\" value=\"" . $arr['id'] . "\" /></td>
<td><a href=" . $config["SITEURL"] . "/contactstaff/staffbox?action=deletestaffmessage&id=$arr[id]>Del</a></td>");
 print("</tr></tbody>\n");
        }
        print("</table>\n");
...
if ($action == "viewpm") {
    ...
    if ($arr4["answered"] == '0') {
        ...
        $answered = "<font color=blue><b>Yes</b></font> by <a href=users/read?id=$answeredby>".class_user_colour($arr5['username'])."</a> (<a href=" . $config["SITEURL"] . "/contactstaff/staffbox?action=viewanswer&pmid=$pmid>Show Answer</a>)";
    }
    if ($arr4["answered"] == '0') {
        $setanswered = "[<a href=" . $config["SITEURL"] . "/contactstaff/staffbox?action=setanswered&id=$arr4[id]>Mark Answered</a>]";
    } else {
        $setanswered = "";
    }
    $iidee = $arr4["id"];
...
<br><table class=main width=730 border=1 cellspacing=0 cellpadding=10><tr><td class=staffpms>\n");
    ...
    print(($arr4["sender"] ? "<a href=" . $config["SITEURL"] . "/contactstaff/staffbox?action=answermessage&receiver=" . $arr4["sender"] . "&answeringto=$iidee><b>Reply</b></a>" : "<font class=gray><b>Reply</b></font>") . " | <a href=" . $config["SITEURL"] . "/contactstaff/staffbox?action=deletestaffmessage&id=" . $arr4["id"] . "><b>Delete</b></a></td>");
    if ($spam == 1)
...
if ($action == "viewanswer") {
    ...
    if (is_valid_id($arr4["sender"])) {
        $res2 = DB::run("SELECT username FROM users WHERE id=" . $arr4["sender"]);
        $arr2 = $res2->fetch(PDO::FETCH_ASSOC);
        $sender = "<a href=" . $config["SITEURL"] . "/users/profile?id=" . $arr4["sender"] . ">" . ($arr2["username"] ? $arr2["username"] : "[Deleted]") . "</a>";
    } else
        $sender = "System";
    if ($arr4['subject'] == "") {
        $subject = "No subject";
    ...
    $elapsed = get_elapsed_time(sql_timestamp_to_unix_timestamp($arr4["added"]));
    print("<table width=750 border=1 cellspacing=0 cellpadding=10 style='margin-bottom: 10px'><tr><td class=text>\n");
    print("<b><a href=$config[SITEURL].php?id=$answeredby>".class_user_colour($arr5['username'])."</a></b> answered this message sent by $sender");
...
<div align=center>
<center></center><b>Answering to <a href=".$config["SITEURL"]."/contactstaff/staffbox?action=viewpm&pmid=<?php echo $array['id'];?>><i><?php echo $array["subject"];?></i></a> sent by <i><?php echo $user["username"];?></i></b></center>
...
if ($action == "takeanswer") {
    ...
    autolink($config["SITEURL"] . '/contactstaff/staffbox', $smsg);
    die;
}
...
if ($action == "setanswered") {
    ...
    autolink($config["SITEURL"] ."/contactstaff/staffbox?action=viewpm&pmid=$id", $smsg);
    die;
}
...
//DB::run("INSERT INTO `staffmessages` (`sender`, `added`, `msg`, `subject`) VALUES (?,?,?,?)", [($_SESSION ? $_SESSION['id'] : 0), get_date_time(), $_POST['msg'], $_POST['subject']]);
DB::run("INSERT INTO `staffmessages` (`sender`, `added`, `msg`, `subject`) VALUES (?,?,?,?)", [($_SESSION['id'] ?: 0), get_date_time(), $_POST['msg'], $_POST['subject']]);
