...
// $count = $row[0];
	...
	if ($count == 0){
		...
		while ($arr = $res->fetch(PDO::FETCH_LAZY)){
			...
			print("<tr><td class='table_col1'>".utc_to_tz($arr['added'])."</td><td align='left' class='table_col2'>$arr[mail_domain]</td><td align='left' class='table_col1'><a href='".TTURL."/users?id=$a4[id]'>$a4[username]"."</a></td><td align='left' class='table_col2'>$arr[comment]</td><td class='table_col1'><a href='/admincp?action=emailbans&amp;remove=$arr[id]'>Remove</a></td></tr>\n");
		}
