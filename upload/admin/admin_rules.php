...
if ($action=="rules" && $do=="view"){
	...
	print("<center><a href='/admincp?action=rules&amp;do=addsect'>Add New Rules Section</a></center><br />\n");
	while ($arr=$res->fetch(PDO::FETCH_LAZY)){
        ...
        print("<form method='post' action='/admincp?action=rules&amp;do=edit'>");
...
if ($action=="rules" && $do=="edit"){
	...
	if ($_GET["save"]=="1"){
		...
		show_error_msg(T_("COMPLETE"), "Rules edited ok<br /><br /><a href='/admincp?action=rules&amp;do=view'>Back To Rules</a>",1);
		die;
	}
	...
	print("<form method=\"post\" action=\"/admincp?action=rules&amp;do=edit&amp;save=1\">");
...
if ($action=="rules" && $do=="addsect"){
	...
	if ($_GET["save"]=="1"){
		...
		show_error_msg(T_("COMPLETE"), "New Section Added<br /><br /><a href='/admincp?action=rules&amp;do=view'>Back To Rules</a>",1);
		die();
	}
	stdhead(T_("SITE_RULES_EDITOR"));
	navmenu();
