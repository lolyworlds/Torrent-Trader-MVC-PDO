...
if ($action=="polls" && $do=="view"){
	...
	echo "<center><a href='$config[SITEURL]/admincp?action=polls&amp;do=add'>Add New Poll</a>&nbsp;/&nbsp;";
	
	echo "<a href='$config[SITEURL]/admincp?action=polls&amp;do=results'>View Poll Results</a></center>";
	...
	while($row = $query->fetch(PDO::FETCH_ASSOC)){
		echo "<a href='$config[SITEURL]/admincp?action=polls&amp;do=add&amp;subact=edit&amp;pollid=$row[id]'>".stripslashes($row["question"])."</a> - ".utc_to_tz($row['added'])." - <a href='$config[SITEURL]/admincp?action=polls&amp;do=delete&amp;id=$row[id]'>Delete</a><br />\n\n";
	}
    echo '</div>';
	end_frame();
